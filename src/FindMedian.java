...
public class FindMedian
{
    ...
    {
        int mid = first + (last - first) / 2; //finds middle
        sortFirstMiddleLast(a, first, mid, last); // sorts the given indexs

        int pivotIndex = mid; //middle is now the sorted pivot index

        if (last - first + 1 > 3) // if the array is greater than 3
        {
            ...
            swap(a, mid, last - 1); //swap to put pivot at last-1 (the setup)
            pivotIndex = last - 1; //reasign pivot index to correct location
            T pivotValue = a[pivotIndex]; // hold pivot value
            ...
            int indexFromLeft = first + 1; //First is sorted, first + 1 is starting point for left
            int indexFromRight = last - 2; //last & last-1 are both sorted, so start at last-2

            boolean done = false;
            while (!done) //while the process is not done
            {
                ...
                while (a[indexFromLeft].compareTo(pivotValue) < 0) // while last index is less than pivot, increment
                    indexFromLeft++;
                ...
                while (a[indexFromRight].compareTo(pivotValue) > 0) // while index from right is greater than pivot, decrement
                    indexFromRight--;
    ...
    T findMedianValue(T[] a, int medianIndex, int first, int last)
    {
        ...
        if(pivotIndex < medianIndex)
        {
            return findMedianValue(a, medianIndex, pivotIndex+1, last);
        }
        return findMedianValue(a, medianIndex,first,pivotIndex-1 );



        //TODO Project 1 - FindMedianValue - Done
